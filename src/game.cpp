...
void KeyCache::populate_nonchanging()
{
	...
	key[GameKeyType::XRAY] = KeyPress("KEY_KEY_X");
	key[GameKeyType::KILLAURA] = KeyPress("KEY_KEY_G");
}
...
void Game::processKeyInput()
{
	if (wasKeyDown(KeyType::DROP)) {
		...
		toggleNoClip();
	} else if (input->wasKeyDown(KeyPress("X"))) {
		toggleXray();
	} else if (input->wasKeyDown(KeyPress("B"))) {
		toggleFullbright();
	} else if (input->wasKeyDown(KeyPress("G"))) {
		toggleKillaura();
	} else if (input->wasKeyDown(KeyPress("H"))) {
		toggleDamage();
...
void Game::toggleFreeMove()
{
	...
	// DFK-Lite: Sync fly cheat flag
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	if (player)
		player->cheat_fly = free_move;
...
void Game::toggleFast()
{
	...
	// DFK-Lite: Sync speed cheat flag
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	if (player)
		player->cheat_speed = fast_move;
...
void Game::toggleNoClip()
{
	...
	// DFK-Lite: Sync noclip cheat flag
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	if (player)
		player->cheat_noclip = noclip;
...
void Game::toggleXray()
{
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	if (!player) return;
	player->cheat_xray = !player->cheat_xray;
	runData.statustext_time = 0;
	m_statustext = player->cheat_xray ? L"X-Ray (Cheat) enabled" : L"X-Ray (Cheat) disabled";
	
	// DFK-Lite: Refresh meshes to apply X-Ray immediately
	client->getEnv().getClientMap().expireMeshes();
}
...
void Game::toggleFullbright()
{
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	if (!player) return;
	player->cheat_fullbright = !player->cheat_fullbright;
	g_fullbright = player->cheat_fullbright; // Sync with global light override
	runData.statustext_time = 0;
	m_statustext = player->cheat_fullbright ? L"Fullbright enabled" : L"Fullbright disabled";
}
...
void Game::toggleKillaura()
{
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	if (!player) return;
	player->cheat_killaura = !player->cheat_killaura;
	runData.statustext_time = 0;
	m_statustext = player->cheat_killaura ? L"Killaura enabled" : L"Killaura disabled";
}

void Game::toggleDamage()
{
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	if (!player) return;
	player->cheat_damage = !player->cheat_damage;
	runData.statustext_time = 0;
	m_statustext = player->cheat_damage ? L"High Damage enabled (5x)" : L"High Damage disabled";
}
...
void Game::runKillaura(f32 dtime)
{
	LocalPlayer *player = client->getEnv().getLocalPlayer();
	if (!player || !player->cheat_killaura)
		return;

	static float timer = 0;
	timer += dtime;
	float delay = (player->cheat_damage) ? 0.02f : 0.15f;
	if (timer < delay)
		return;
	timer = 0;
	...
	client->getEnv().getActiveObjects(playerpos, 5.0f * BS, objects);

	// Prepare item for direct punch prediction
	ItemStack item;
	u16 wielded_item_index = client->getPlayerItem();
	if (local_inventory) {
		InventoryList *mlist = local_inventory->getList("main");
		if (mlist && wielded_item_index < mlist->getSize())
			item = mlist->getItem(wielded_item_index);
	}
	if (item.name.empty()) {
	...
	for (size_t i = 0; i < objects.size(); i++) {
		...
		if (!obj || obj->isLocalPlayer())
			continue;

		PointedThing pointed;
		pointed.type = POINTEDTHING_OBJECT;
		pointed.object_id = obj->getId();

		// Management of reload timers for continuous damage
		runData.time_from_last_punch = 1.0f; // Force full damage prediction
		
		// Visual feedback and local damage prediction
		v3f objpos = obj->getPosition();
		v3f dir = (objpos - playerpos).normalize();
		obj->directReportPunch(dir, &item, runData.time_from_last_punch);
		
		runData.time_from_last_punch = 0;
		runData.object_hit_delay_timer = 0;

		if (player->cheat_damage) {
			for (int j = 0; j < 5; j++) {
		...
		// Removed 'break' to hit all targets
	}
}
		...
		const v3f &player_position, bool show_debug)
{
	...
	if (isLeftPressed()) {
		...
		if (runData.object_hit_delay_timer <= 0.0) {
			...
			runData.object_hit_delay_timer = (player && player->cheat_damage) ? 0 : object_hit_delay;
		}
