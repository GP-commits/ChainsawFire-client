...
LocalPlayer::LocalPlayer(Client *client, const char *name):
	...
	// DFK-Lite cheat flags
	cheat_speed(false),
	cheat_noclip(false),
	cheat_fly(false),
	cheat_xray(false),
	cheat_fullbright(false),
	cheat_killaura(false),
	cheat_damage(false),
		...
		std::vector<CollisionInfo> *collision_info)
{
	...
	bool free_move = (g_settings->getBool("free_move") && fly_allowed) || cheat_fly || cheat_noclip;

	// DFK-Lite: force noclip regardless of server privileges
	if (cheat_noclip) {
...
void LocalPlayer::applyControl(float dtime)
{
	...
	bool free_move = (fly_allowed && g_settings->getBool("free_move")) || cheat_fly || cheat_noclip;
	bool fast_move = (fast_allowed && g_settings->getBool("fast_move")) || cheat_speed;
	...
	if((!touching_ground && !free_move && !is_climbing && !in_liquid) || (!free_move && m_can_jump && control.jump))
	{
		...
		incV = (cheat_fly || cheat_noclip) ? incH : 0; // Enable vertical acceleration if cheating
	}
	...
	// DFK-Lite: Speed hack multiplier (bypasses server physics override)
	float cheat_speed_mult = cheat_speed ? 5.0f : 1.0f;
	if (control.aux1) cheat_speed_mult *= 2.0f; // Holding E (Aux1) gives 2x boost
	accelerateHorizontal(speedH * physics_override_speed * cheat_speed_mult, incH * physics_override_speed * cheat_speed_mult);
	
	float v_mult = (cheat_fly || cheat_noclip) ? cheat_speed_mult : 1.0f;
	if (free_move)
		accelerateVertical(speedV * physics_override_speed * v_mult, incV * physics_override_speed * v_mult);
}
