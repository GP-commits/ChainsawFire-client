	...
	*/
	{
		...
		counter += dtime;
		float interval = m_recommended_send_interval;
		if (player && (player->cheat_speed || player->cheat_noclip || player->cheat_fly))
			interval = 0.02f; // Send packets more frequently to avoid server rubber-banding

		if ((m_state == LC_Ready) && (counter >= interval))
		{
			counter = 0.0;
			sendPlayerPos();
		}
...
void writePlayerPos(LocalPlayer *myplayer, ClientMap *clientMap, NetworkPacket *pkt, v3f *pos_override = NULL)
{
	v3f pf           = (pos_override ? *pos_override : myplayer->getPosition()) * 100;
...
void Client::sendPlayerPos()
{
	...
	v3f current_pos = myplayer->getPosition();
	v3f old_pos = myplayer->last_position;
	...
	// DFK-Lite: Anti-Cheat Bypass (Packet Stepping)
	// If moved significantly, send intermediate packets to satisfy server speed checks
	if (myplayer->cheat_speed && (current_pos - old_pos).getLength() > 1.5f * BS) {
		v3f diff = current_pos - old_pos;
		float dist = diff.getLength();
		int steps = (int)(dist / (1.5f * BS)); 
		if (steps > 1) {
			if (steps > 6) steps = 6; // Limit to 6 packets per frame
			for (int i = 1; i < steps; i++) {
				v3f intermediate = old_pos + diff * ((float)i / steps);
				NetworkPacket step_pkt(TOSERVER_PLAYERPOS, 12 + 12 + 4 + 4 + 4 + 1 + 1);
				writePlayerPos(myplayer, &map, &step_pkt, &intermediate);
				Send(&step_pkt);
			}
		}
	}

	myplayer->last_position     = current_pos;
	myplayer->last_speed        = myplayer->getSpeed();
	myplayer->last_pitch        = myplayer->getPitch();
	myplayer->last_yaw          = myplayer->getYaw();
	myplayer->last_keyPressed   = myplayer->keyPressed;
